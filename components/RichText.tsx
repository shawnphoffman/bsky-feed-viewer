...
import { AppBskyFeedPost as Post, RichText as RichTextProcessor } from '@atproto/api'
...
export function RichText({ post }: { post: Post.Record }) {
	...
	for (const segment of richtext.segments()) {
		if (segment.isMention()) {
			els.push(<Link href={`/repositories/${segment.mention?.did}`}>{segment.text}</Link>)
...
export function toShortUrl(url: string): string {
	try {
		...
		const shortened = urlp.host + (urlp.pathname === '/' ? '' : urlp.pathname) + urlp.search + urlp.hash
